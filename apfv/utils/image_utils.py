...
def save_image(image, output_path="./out/test.jpg"):
  if image.shape[0] == 3:
...
def draw_boxes(
    image, det_out, class_names,
    from_path=True, show_text=True,
    out_file='temp_bbox_img.png'):
...
def visualize_features(
    intermediate_features, output_dir, file_prefix='',
    data_format='channels_last', image_size=(224, 224), only_first_channel=True):
...
def visualize_features_compare(
    ori_features, adv_features, output_dir, file_prefix='',
    data_format='channels_last', image_size=(224, 224), only_first_channel=True):
  if data_format == 'channels_last':
